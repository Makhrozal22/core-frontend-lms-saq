...
export const ProfileHero = ({ parent, students = [], loadingChildren = false }) => {
  return (
    <div className="bg-gradient-to-r from-emerald-600 to-teal-600 rounded-3xl p-6 flex items-center gap-5 shadow-lg shadow-emerald-900/10 text-white">
      ...
      <div className="min-w-0 flex-1">
        <h2 className="font-bold text-lg truncate">{parent?.name || parent?.full_name || "Wali Santri"}</h2>
        ...
        <div className="flex items-center gap-1.5 mt-2">
          ...
          {loadingChildren ? (
            ...
            <span className="text-emerald-100 text-xs font-medium">
              {students.length} Anak Terdaftar
            </span>
          )}
        </div>
      </div>
